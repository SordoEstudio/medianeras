...
├── mock/                  ← re-exporta desde src/data/ (thin wrappers)
│   ├── talleres.ts
│   ├── equipo.ts
│   ├── config.ts
│   └── testimonios.ts
├── skills/
│   ├── medianeras-client/SKILL.md
│   ├── medianeras-brand/SKILL.md
│   ├── medianeras-content/SKILL.md
│   └── medianeras-cms/SKILL.md
├── src/
│   ├── data/              ← datos reales del cliente (JSON por sección)
│   │   ├── config.json
│   │   ├── equipo.json
│   │   ├── actividades.json
│   │   └── testimonios.json
...
│   └── lib/
│       ├── cms.ts         ← cliente del CMS API (getActividades, getEquipo, getConfig, getTestimonios)
│       └── types.ts       ← tipos compartidos (Actividad con docentes[], Testimonio)
...
|---|---|---|
| **Quién es la cliente, copy real, contexto** | `medianeras-client` | Antes de escribir cualquier texto o tomar decisiones de contenido |
